...
C1 — ats_production_governor.py; REMOVED 2026-08-19, superseded by ats_governor_fixed.py (44KB), which carries forward same-named, more advanced versions of ATSGovernorProduction, StreamingBiasMonitor, SafeguardVerifier, LegalEvidencePackager, and BiasNeutralizationEngine, plus DecisionFunctionAnalyzer in place of DecisionFunctionInverter (now backed by ats_statistics.py's StatisticalBiasDetector — chi-squared/Fisher's exact hypothesis testing — instead of hard-coded gap thresholds). Two capabilities present in the original C1 were NOT carried forward and are now confirmed gaps: name-proxy (vowel-ratio) bias detection and SafeguardVerifier.verify_independent_validation (see S5, S7). All "C1" references below now point to ats_governor_fixed.py unless marked otherwise. [OBSERVED]

C2 — ats_counter_system.py; executable module containing CounterATS, DecisionFunctionInverter, SyntheticValidationBreaker, AuditReportValidator, and BiasNeutralizationEngine. [OBSERVED]

C3 — ats_governance_kernel_bridge.py; executable bridge module containing CandidateSignals, ATSKernelAdapter, forensic policy factories, manifest construction, ATSGovernorKernel, and a module entry point. [OBSERVED]

C4 — ats_gsa_core.py; text artifact containing Python source for ATSGovernanceCore and its component classes. [OBSERVED]

C5 — ats_governance_kernel_bridge_source_copy.txt; text artifact with the same SHA-256 digest as C3. [OBSERVED]

C6 — ats_governance_kernel_bridge_reference.md; Markdown artifact containing a formatted bridge variant; its digest differs from C3. [OBSERVED]
...
C8 — ats_governor_package.md; package documentation containing quick-start, deployment, checklist, and legal-disclaimer sections. [OBSERVED]

C9 — ats_governor_integration_guide.md; integration documentation containing pipeline, alerting, audit, archive, and deployment references. [OBSERVED]

C10 — ats_governor_operations_toolkit.md; operational toolkit documentation containing candidate fields, statistical checks, validation checks, and workflow descriptions. [OBSERVED]
...
C12 — ats_bias_decision_framework.md; decision-framework documentation containing bias-level routing and remediation descriptions. [OBSERVED]
...
C14 — ats_application_tracking_system_auditor.pdf; PDF artifact, identified as an eight-page PDF file. [OBSERVED]

C15 — ats_hidden_bias_counter_analysis.pdf; PDF artifact, identified as an eight-page PDF file. [OBSERVED]
...
C24 — text-artifact governance contract; C4 defines SystemConfig and AuditLogger record fields and uses them across ATSGovernanceCore components. [OBSERVED]
...
LOCATION: ats_governor_fixed.py, StreamingBiasMonitor.ingest_decision — carried forward under the same method name after ats_production_governor.py was removed 2026-08-19. [OBSERVED]
...
LOCATION: ats_governor_fixed.py, StreamingBiasMonitor.__init__, ingest_decision, and analyze_batch — carried forward under the same method names after ats_production_governor.py was removed 2026-08-19. [OBSERVED]
...
ENFORCEMENT: Hard-coded predicates, rejection counts, and threshold comparisons in _gap (renamed from _compute_rejection_gap_by) and analyze_batch. [OBSERVED]
LOCATION: ats_governor_fixed.py, StreamingBiasMonitor.analyze_batch and _gap — carried forward after ats_production_governor.py was removed 2026-08-19, with the helper renamed from _compute_rejection_gap_by to _gap. Logic and thresholds (0.15/0.30) are otherwise unchanged; this streaming path was left as a fast heuristic on purpose — its own code comment notes "use p-value for significance" for the slower, statistically rigorous path (DecisionFunctionAnalyzer, seam-equivalent to old S9-S12, now StatisticalBiasDetector-backed). [OBSERVED]
...
SEAM ID: S4 [STATUS: REMOVED — see note]
NAME: Employment-volatility rejection-gap detection
TYPE: DATA; secondary TRUST [OBSERVED, historical]
SIDE A: C1 / StreamingBiasMonitor.analyze_batch [OBSERVED, historical]
SIDE B: C1 / employment-volatility gap calculation [OBSERVED, historical]
WHAT CROSSES: employment_history length, years_professional, and REJECTED decisions crossed into a stable/volatile comparison. [OBSERVED, historical]
CONTRACT: Volatile was employment-history length divided by max(years_professional, 1) greater than 1.5; a gap greater than 0.15 produced a signature. [OBSERVED, historical]
ENFORCEMENT: Hard-coded ratio, group predicate, and threshold checks. [OBSERVED, historical]
LOCATION: was ats_production_governor.py (StreamingBiasMonitor.analyze_batch and _compute_rejection_gap_by) — file removed 2026-08-19, superseded by ats_governor_fixed.py. The new StreamingBiasMonitor.analyze_batch only computes the geographic gap (see S3); no employment_history/volatility check remains anywhere in that method. This is a capability gap introduced by the supersession, not a rename. [OBSERVED]
ORIGIN: README and toolkit documentation name employment volatility as a demographic proxy or bias signal. [DOCUMENTED]
DEPENDS ON: S2 [OBSERVED, historical]
FAILURE MODE: Missing or incompatible employment fields prevented the ratio or predicate from completing; a detected gap produced an alert event. [OBSERVED, historical]
CONFIDENCE: high — the ratio and thresholds were directly readable in the removed file; the gap in the current codebase is confirmed by direct search, not inference. [OBSERVED]

SEAM ID: S5 [STATUS: REMOVED — see note]
NAME: Name-proxy rejection-gap detection
TYPE: DATA; secondary TRUST [OBSERVED, historical]
SIDE A: C1 / StreamingBiasMonitor.analyze_batch [OBSERVED, historical]
SIDE B: C1 / name-proxy gap calculation [OBSERVED, historical]
WHAT CROSSES: Candidate name values and REJECTED decisions crossed into a vowel-ratio group comparison. [OBSERVED, historical]
CONTRACT: The proxy group was vowel_ratio(name) greater than 0.4; a gap greater than 0.15 created a CRITICAL, action-required signature. [OBSERVED, historical]
ENFORCEMENT: _vowel_ratio and hard-coded threshold branches. [OBSERVED, historical]
LOCATION: was ats_production_governor.py (StreamingBiasMonitor.analyze_batch and _vowel_ratio) — file removed 2026-08-19, superseded by ats_governor_fixed.py. No _vowel_ratio or equivalent name-proxy check exists in ats_governor_fixed.py, ats_statistics.py, or ats_embeddings.py (confirmed by search — zero matches for "vowel" across all three). This is a capability gap introduced by the supersession, not a rename: name-based proxy discrimination detection is currently absent from the codebase. [OBSERVED]
ORIGIN: README and toolkit documentation name name-based or demographic proxy detection. [DOCUMENTED]
DEPENDS ON: S2 [OBSERVED, historical]
FAILURE MODE: A missing or non-string name prevented vowel-ratio processing; a detected gap was stored as a critical alert. [OBSERVED, historical]
CONFIDENCE: high — the function and branch were directly readable in the removed file; the gap in the current codebase is confirmed by direct search, not inference. [OBSERVED]
...
LOCATION: ats_governor_fixed.py, ATSGovernorProduction.process_hiring_decision — carried forward under the same method name after ats_production_governor.py was removed 2026-08-19. [OBSERVED]
...
SEAM ID: S7 [STATUS: PARTIALLY CARRIED FORWARD — see note]
...
WHAT CROSSES: Scoring-function source text and candidate data at scoring cross into verification methods. [OBSERVED]
CONTRACT: Geographic code is checked for listed location patterns; candidate keys are checked for name fields. [OBSERVED]
ENFORCEMENT: String-pattern scans and key inspection. [OBSERVED]
LOCATION: ats_governor_fixed.py, SafeguardVerifier.verify_geographic_blind_scoring and verify_name_anonymization — both carried forward under the same method names after ats_production_governor.py was removed 2026-08-19. verify_independent_validation did NOT carry forward: zero matches for that name in ats_governor_fixed.py. The "independent validation returns a NEEDS_VERIFICATION result with unverified checks" part of this seam's original CONTRACT no longer applies to the current codebase — this is a capability gap, not a rename. [OBSERVED]
...
CONFIDENCE: high — verifier branches and return values are directly readable; the missing method is confirmed by direct search, not inference. [OBSERVED]
...
LOCATION: ats_governor_fixed.py, ATSGovernorProduction.run_full_audit — carried forward under the same method name after ats_production_governor.py was removed 2026-08-19. [OBSERVED]
...
LOCATION: ats_counter_system.py, DecisionFunctionInverter.ingest_decisions and CounterATS.audit. [OBSERVED]
...
LOCATION: ats_counter_system.py, DecisionFunctionInverter.detect_correlated_rejection_patterns. [OBSERVED]
...
LOCATION: ats_counter_system.py, SyntheticValidationBreaker.analyze_validation_circularity. [OBSERVED]
...
LOCATION: ats_counter_system.py, AuditReportValidator.validate_audit_report. [OBSERVED]
...
LOCATION: ats_counter_system.py, BiasNeutralizationEngine.apply_corrections and propose_system_safeguards. [OBSERVED]
...
LOCATION: ats_counter_system.py, CounterATS.audit. [OBSERVED]
...
LOCATION: ats_governance_kernel_bridge.py, CandidateSignals and ATSKernelAdapter.to_event_delta. [OBSERVED]
...
LOCATION: ats_governance_kernel_bridge.py, ATSKernelAdapter.build_provenance and commit. [OBSERVED]
...
LOCATION: ats_governance_kernel_bridge.py, ATSGovernorKernel.evaluate_candidate. [OBSERVED]
...
LOCATION: ats_governance_kernel_bridge.py, make_keyword_stuffing_policy, make_low_confidence_policy, make_synthetic_alignment_policy, make_geo_anomaly_policy, build_forensic_policy_vm. [OBSERVED]
...
LOCATION: ats_governance_kernel_bridge.py, build_ats_manifest and ATSGovernorKernel.evaluate_candidate. [OBSERVED]
...
LOCATION: ats_governance_kernel_bridge.py, ATSGovernorKernel.__init__ and evaluate_candidate; default /tmp/ats_kernel_bridge.log. [OBSERVED]
...
LOCATION: ats_governance_kernel_bridge.py, ATSGovernorKernel.replay_audit_trail and module entry-point harness. [OBSERVED]
...
SEAM ID: S27
NAME: SystemConfig to ATS Governance Core components
TYPE: DATA; secondary STATE [OBSERVED]
SIDE A: C24 / SystemConfig [OBSERVED]
SIDE B: C4 / ATSGovernanceCore components [OBSERVED]
...
LOCATION: ats_gsa_core.py, SystemConfig and ATSGovernanceCore.__init__/process_candidate_ingress. [OBSERVED]
...
SIDE A: C4 / ATSGovernanceCore components [OBSERVED]
...
LOCATION: ats_gsa_core.py, AuditLogger. [OBSERVED]
...
LOCATION: ats_gsa_core.py, SkillOntology and JDRequirementExtractor. [OBSERVED]
...
LOCATION: ats_gsa_core.py, ResumeSubstanceValidator. [OBSERVED]
...
LOCATION: ats_gsa_core.py, DomainTranspiler.register_industry and transpile. [OBSERVED]
...
TYPE: CONTROL; secondary DATA [OBSERVED]
SIDE A: C4 / ATSGovernanceCore candidate processing [OBSERVED]
...
LOCATION: ats_gsa_core.py, TandemAlphaBetaBridge.evaluate. [OBSERVED]
...
LOCATION: ats_gsa_core.py, MasterGovernanceControl.route_mid_band; human-review destination is documented in package and integration materials. [OBSERVED]
...
LOCATION: ats_gsa_core.py, RealityAnchorGate.validate_synthetic_packet and GammaRedTeam. [OBSERVED]
...
SIDE B: C4 / ATSGovernanceCore.process_candidate_ingress [OBSERVED]
...
LOCATION: ats_gsa_core.py, ATSGovernanceCore.process_candidate_ingress. [OBSERVED]
...
TYPE: DATA; secondary IDENTITY [OBSERVED]
SIDE A: C4 / ATSGovernanceCore [OBSERVED]
...
LOCATION: ats_gsa_core.py, ATSGovernanceCore._transparency_directive and process_candidate_ingress. [OBSERVED]
...
LOCATION: ats_threat_model.md schemas; ats_governor_fixed.py (successor to ats_production_governor.py, removed 2026-08-19) and ats_counter_system.py analyzer methods. [OBSERVED]
...
LOCATION: README.md, ats_governor_integration_guide.md, ats_governor_package.md, and C1/C2 packaging methods. [OBSERVED]
...
SIDE B: C1 / ats_governor_fixed.__main__ [OBSERVED]
...
LOCATION: ats_governor_fixed.py, module entry point — carried forward after ats_production_governor.py was removed 2026-08-19. [OBSERVED]
...
SIDE B: C4 / ATSGovernanceCore.run_red_blue_tests [OBSERVED]
...
LOCATION: ats_gsa_core.py, final module-entry block and run_red_blue_tests. [OBSERVED]
